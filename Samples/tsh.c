...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        { // the prompt would not be printed.
            //    printf ("found -p\n"); // For debug
            emit_prompt = 0;
        }
    }
    ...
    while (1)
    {
        ...
        // fgets
        fgets(cmdline, MAXLINE, stdin); // now cmdline has sth;
...
void eval(char *cmdline)
{
    ...
    // add signal set to mask
    sigemptyset(&mask);
    sigaddset(&mask, SIGCHLD);
    sigaddset(&mask, SIGINT);
    sigaddset(&mask, SIGTSTP);

    bg = parseline(cmdline, argv); // now bg has it's state;

    if (argv[0] == NULL) // ignore if there's no cmd
        return;

    if (!builtin_cmd(argv))
    { // this is not a build in cmd
        ...
        { // this is a child
            ...
            setpgid(0, 0); //New jobs should have new process IDs

            // Prob 1: what does execev do?
        ...
        {
            // not a child, it's a parent

            if (!bg)
            { // foreground
                // printf("it's a foreground\n");
            ...
            { // it's a background
                // printf("it's a background\n");
                addjob(jobs, pid, BG, cmdline);
                // unblock the signal mask - actually unnecessary.....
                sigprocmask(SIG_UNBLOCK, &mask, NULL);
            }
        }
    }
...
void waitfg(pid_t pid)
{
    ...
    /* WUNTRACED does work here. Seems that after
    the job's state change to ST, it no longer return 
    something. */
    ...
    if(!currentJob){   
        printf("No foreground job to wait\n");                                                          
        return;                                                                   
    }
    while(currentJob -> pid != 0 && currentJob -> state != ST){ // job exists and it's not stopped  
    }
    return;
}
...
{
    // TODO
    pid_t jobPID = fgpid(jobs); // get PID of the foregorund job

    if (jobPID != 0)
    { // foreground job exists
        // kill it with according PID
        kill(-jobPID, SIGINT);
        // delete it from the jobs list
        deletejob(jobs, jobPID);
    }
    else
    {
        printf(" No foreground job is ongoing\n");
    }
...
void sigtstp_handler(int sig)
{
    // // TODO
    pid_t jobPID;
    jobPID = fgpid(jobs); // get the  PID of foregorund job
    struct job_t *currentJob;   // create a package for current job
    currentJob = getjobpid(jobs, jobPID);

    if (jobPID != 0)
    { // the job exists
        // get this job
        // currentJob = getjobpid(jobs, jobPID);
        // kill the job with given PID
        kill(-jobPID, SIGTSTP);
        // update its state
        currentJob->state = ST;
    }
